...
class ModelComparisoner:
            ...
            "required": {
                ...
                "ckpt_names": (
                    "STRING",
                    {
                        "multiline": True,
                        "default": "",
                        "tooltip": "List of additional checkpoint filenames, one per line or comma separated",
                    },
                ),
                ...
                "sampling_override": (
                    ["default", "eps", "v_prediction", "lcm", "x0", "sd3", "flux"],
                ),
                ...
                "generate_grid": (
                    ...
                    {
                        "default": True,
                        "tooltip": "Generate the grid of images (decodes with each model's VAE). Turn off to save memory/time if you only need Latents.",
                    },
                ),
                "image_mode": (
                    ["grid", "batch"],
                    {
                        "default": "grid",
                        "tooltip": "grid: Concatenate images horizontally. batch: Stack images (N individual images).",
                    },
                ),
                "latent_mode": (
                    ["batch", "grid"],
                    {
                        "default": "batch",
                        "tooltip": "batch: Stack latents (N images). grid: Concatenate latents horizontally (1 wide image).",
                    },
                ),
            },
            "optional": {
                ...
                "vae": (
                    "VAE",
                    {
                        "tooltip": "Optional VAE to use for decoding. If provided, this VAE is used for ALL models instead of the checkpoint's built-in VAE."
                    },
                ),
            },
        }
    ...
    def compare_models(
        ...
        **kwargs,
        ...
        names = []
        if ckpt_name_1 != "None":
            names.append(ckpt_name_1)

        # Add dynamically added checkpoints from frontend
        for key, value in kwargs.items():
            if key.startswith("ckpt_name_") and value != "None" and value not in names:
                names.append(value)

        if "\n" in ckpt_names:
            extra_names = [x.strip() for x in ckpt_names.split("\n") if x.strip()]
        else:
            extra_names = [x.strip() for x in ckpt_names.split(",") if x.strip()]
        ...
        for i, ckpt_name in enumerate(names):
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"ModelComparisoner: Checkpoint {ckpt_name} not found. Skipping."
                    )
                    continue
                ...
                if sampling_override != "default":
                    ...
                    if sampling_override in ["eps", "v_prediction", "lcm", "x0"]:
                        ...
                        model_sampling = ModelSamplingAdvanced(
                            model.model.model_config, zsnr=zsnr
                        )
                ...
                if noise is None:
                    if not add_noise:
                        noise = torch.zeros(
                            work_latent.size(),
                            dtype=work_latent.dtype,
                            layout=work_latent.layout,
                            device="cpu",
                        )
                    else:
                        batch_inds = latent_image.get("batch_index", None)
                        noise = comfy.sample.prepare_noise(
                            work_latent, noise_seed, batch_inds
                        )
                    noise_for_model = noise
                else:
                    if noise.shape != work_latent.shape:
                        if not add_noise:
                            noise_for_model = torch.zeros(
                                work_latent.size(),
                                dtype=work_latent.dtype,
                                layout=work_latent.layout,
                                device="cpu",
                            ...
                            batch_inds = latent_image.get("batch_index", None)
                            noise_for_model = comfy.sample.prepare_noise(
                                work_latent, noise_seed, batch_inds
                            )
                    else:
                        noise_for_model = noise
                ...
                if generate_grid:
                    # Decode
                    decoded_images = decode_node.decode(
                        vae_to_use, {"samples": samples}
                    )[0]
                    ...
                    for img_tensor in decoded_images:
                        ...
                        if row_label_height > 0:
                            w, h = img.size
                            new_img = Image.new(
                                "RGB", (w, h + row_label_height), (0, 0, 0)
                            )
